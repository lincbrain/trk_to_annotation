...
def split_along_grid(streamline_start, streamline_end, streamline_tract, streamline_scalars, lb, ub, grid_densities):
    ...
    for a in range(3):
        ...
        if using_scalars:
            ...
            streamline_scalars_expand = np.repeat(np.expand_dims(streamline_scalars, axis=1), 2, axis=1)
        ...
        if using_scalars:
            ...
            streamline_scalars_expand[not_same_grid] = np.repeat(np.expand_dims(streamline_scalars_expand[not_same_grid][:, 0], axis=1), 2, axis=1)
        ...
        if using_scalars:
            streamline_scalars = np.reshape(streamline_scalars_expand, (-1, streamline_scalars.shape[1]))
            streamline_scalars = streamline_scalars[np.invert(np.isnan(streamline_scalars[:, 0]))]
    ...
    while np.any(streamline_tract == tract_id):
        ...
        with open(tract_file, 'wb') as f:
            ...
            for i in range(len(lines[indexes])):
                ...
                f.write(np.asarray((orr/np.linalg.norm(orr))*255, dtype='<u1').tobytes())
...
def write_spatial_and_info(lines, lb, ub, grid_densities, streamline_tracts, streamline_scalars, scalar_keys, output_dir):
    ...
    for density_index in range(len(grid_densities)):
        prob = min(LIMIT/(lines.shape[0]/(grid_densities[density_index]**3)), 1.0)
    ...
    for density_index in range(len(grid_densities)):
        ...
        spatial_index = {f"{x}_{y}_{z}": [np.array([]),np.array([]),np.array([]),np.array([])] for x in range(grid_shape[0]) for y in range(grid_shape[1]) for z in range(grid_shape[2])}
        cells = np.floor((((streamline_start + streamline_end)/2 - lb)/dimensions)*grid_shape).astype(int)
                ...
                for j in range(grid_shape[1]):
                    ...
                    if np.any(cells_xy):
                        for k in range(grid_shape[2]):
                            ...
                            spatial_index[f"{i}_{j}_{k}"] = [lines[cells_xyz], ids[cells_xyz], streamline_tracts[cells_xyz], streamline_scalars[cells_xyz]]
        ...
        for cell_key, annotations in spatial_index.items():
            ...
            selected_lines = np.isin(annotations[2], np.array(range(number_tracts))[tract_level == density_index])
